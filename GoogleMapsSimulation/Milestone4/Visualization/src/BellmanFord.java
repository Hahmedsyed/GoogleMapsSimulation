...
class City {
    ...
    double latitude;
    double longitude;
    LocalDateTime eta; // New field for ETA
...
public class BellmanFord {
    ...
    public void readDataFromCSV(String csvFilePath) {
        try (BufferedReader br = new BufferedReader(new FileReader(csvFilePath))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                addEdge(new City(cityOrigin, stateOrigin), new City(cityDestination, stateDestination), distance, speed,
                        gradient);
            }
    ...
    public static void writeCityAttributesToCSV(List<List<String>> cityAttributes, String csvFilePath) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(csvFilePath))) {
            writer.write("City,State,Distance,Gallons,Weather,Latitude,Longitude,ETA,Risk_Factor\n");
    ...
    public static void main(String[] args) {
        ...
        if (previousNodes.containsKey(destination)) {
            ...
            if (!bellmanFord.visitedCities.contains(origin)) {
                ...
                double[] weatherInfo = bellmanFord.checkWeatherCondition(distance,
                        bellmanFord.getWeatherCondition(origin, currentDateTime));
                distance = weatherInfo[0];
                risk_factor = (int) weatherInfo[1];
                ...
                attributes.add(currentDateTime.format(DateTimeFormatter.ofPattern("MM/dd/yyyy HH:mm")));
                attributes.add(String.valueOf(risk_factor));
                ...
                if (i > 0) {
                    ...
                    for (Edge edge : bellmanFord.graph.get(shortestPath.get(i - 1))) {
                        if (edge.destination.equals(city)) {
                            ...
                            if (cityWeather != null) {
                                String weather = bellmanFord.getWeatherCondition(city, currentDateTime);
                                double[] weatherInfo = bellmanFord.checkWeatherCondition(distance,
                                        weather);
                                distanceWeather = weatherInfo[0];
                                risk_factor = (int) weatherInfo[1];
                                System.out
                                        .println("Weather: " + bellmanFord.getWeatherCondition(city, currentDateTime));
                                System.out.println("risk_factor: " + risk_factor);
                                ...
                                if (!bellmanFord.visitedCities.contains(city)) {
                                    ...
                                    System.out.printf("--distance %.2f miles,--- Risk Factor %d", distance,
                                            risk_factor);
                                    System.out.printf("--ETA: %s ",
                                            currentDateTime.format(DateTimeFormatter.ofPattern("MM/dd/yyyy HH:mm")));
                                    System.out.printf("--Gallons required: %s -- Weather: %s", formattedGallons,
                                            bellmanFord.getWeatherCondition(city, currentDateTime));
            ...
            System.out.printf("%-20s%-20s%-20s%-20s%-20s%-20s%-20s%-20s\n", "City", "State", "Distance", "Gallons",
                    "Weather", "Latitude", "Longitude", "ETA", "Risk_Factor");

            for (List<String> attributes : BellmanFord.cityAttributes) {
                System.out.printf("%-20s%-20s%-20s%-20s%-20s%-20s%-20s%-20s%-20s\n", attributes.get(0),
                        attributes.get(1), attributes.get(2), attributes.get(3), attributes.get(4), attributes.get(5),
                        attributes.get(6), attributes.get(7), attributes.get(8));
            }
